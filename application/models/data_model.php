...
class Data_model extends CI_Model {
    ...
    function getModelos($limite){
        $query = $this->db->query("SELECT m.id_modelo, m.nombre,m.pais,m.marcas,m.modelo,m.productID, ma.nombre AS marca, ma.id_marca AS id_marca
    ...
    function getCar($id, $pais) {
        $sql = "SELECT
        distinct(auto.id_automovil),
        marca.nombre as marca,
        mdl.nombre as modelo,
        compra.cuota_mensual as cuota_com,
        renting.cuota_diaria as cuota_ren
        ...
        auto.id_automovil='".$id."' AND auto.pais='".$pais."'";
    ...
    function getInfoCar($id){
        ...
        renting.cargo_gps as cargo_gps,
        renting.cargo_seguro as cargo_seguro,
    ...
    function cargarPais(){
        try {
            ...
            $pais = $client->consulta_pais($token);
            for ($i=0; $i <count($pais); $i++) {
              $paises[$i] = explode("-", $pais[$i]);
            }
