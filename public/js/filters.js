$(function () {
	$('.filters-open').on('click', function (e) {
		e.preventDefault();
		$('.filters').dequeue().slideToggle('fast', function () {
			var is_visible = $('.filters:visible').length > 0;
			$('.filters')[is_visible ? 'addClass' : 'removeClass']('opened').css('display', '');
		});
	});
});

function filter_charact_ta(lapso_actual, pagina_actual,vista_actual,orden, pais){
